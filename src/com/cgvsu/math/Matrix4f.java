...
public class Matrix4f {
    ...
    public Vector3f multiplyMatrixByVector(Vector3f vector) {
        ...
        for (int i = 0; i < 4; i++) {
            result[i] = data[i][0] * vector.getX() + data[i][1] * vector.getY() +
                    data[i][2] * vector.getZ() + data[i][3];
        }
    ...
    public Matrix4f(Matrix4f other) {
        ...
        for (int i = 0; i < 4; i++) {
            this.data[i] = Arrays.copyOf(other.data[i], 4);
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                if (Math.abs(data[i][j] - matrix4F.data[i][j]) > EPSILON) {
                    return false;
                }
            }
        }
